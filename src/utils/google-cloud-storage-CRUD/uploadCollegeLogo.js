...
export const uploadCollegeLogo = async (localFilePath) => {
  try {
    // Ensure you use the same environment variable name as your NGO function
    ...
    // Generate a unique destination name inside a 'college-logos' folder
    const destination = `college-logos/${Date.now()}_${path.basename(localFilePath)}`;
    ...
    const [file] = await storage.bucket(bucketName).upload(localFilePath, {
      ...
      metadata: { contentType: "image/jpeg" }, // Hints browser to display image
    });
    ...
    console.error("Cloud Upload Error (College):", error.message);
